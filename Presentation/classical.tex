...
\frame{\frametitle{Косинусный закон преломления}
\begin{figure}[!htb]
\minipage{0.32\textwidth}
\includegraphics[width=\linewidth]{images/ch4/section1/example1.pdf}
%    \caption{$\cos \theta_j = \dfrac{n_i}{n_j} \cos \theta_i$}
%    \label{fig:sect1_example1}
$\cos \theta_j = \dfrac{n_i}{n_j} \cos \theta_i$
\endminipage\hfill
\minipage{0.32\textwidth}
...
Полное внутреннее отражение, если $\theta_j$ не определен.
\endminipage\hfill
\minipage{0.32\textwidth}
...
%[beginpenalty=10000] % https://tex.stackexchange.com/a/476052/104425
  \item Доказать существование постоянной движения для  классического бильярда с косинусным законом преломления на границе раздела сред на софокусных столах.
...
\begin{frame}\frametitle{\normalsize{Существование первого интеграла (нет пересечений разделяющих квадрик)}}
%
...
Фиксированной паре $\lambda_1$ и $\dfrac{n_{in}}{n_{out}}$ соответствует отрезок на диаграмме.
...
%    \label{fig:sect2_problemAbifurcations}
    ...
    \centering
    	\begin{figure}[!htb]
\includegraphics[width=7cm]{section2/long red line bifurcations small.png}
%    \caption{Область значений $\Xi$ --- отрезок на красной прямой.}
        	\end{figure}
	    Постоянная движения $\Xi$ принимает значения на отрезке 
	\end{columns}
\end{frame}
...
\begin{tabular}{|c|c|c|}
\hline 
$D_1^1$  	& 	$\alpha_{in} \in (\lambda_1, b^2), \ \alpha_{out} \in (b^2, a^2)$			& сфера с 5 ручками и проколами; \\ \hline 
$D_1^2$  	& 	$\alpha_{in} \in (b^2, a^2), \ \alpha_{out} \in (\alpha_{in}, a^2)$				& сфера с 5 ручками  и проколами; \\ \hline 
$D_1^3$  	& 	$\alpha_{in} \in (b^2, a^2), \ \alpha_{out} \in (b^2, \alpha_{in})$				& сфера с 5 ручками  и проколами; \\ \hline 
...
$D_1^6$  	& 	$\alpha_{in} \in (b^2, a^2), \ \alpha_{out} \in (\lambda_1, b^2)$			& сфера с 5 ручками  и проколами; \\ \hline 
\hline
...
Тип поверхности сохраняется внутри каждой ячейки диаграммы.

Нерегулярные значения $\Xi$ --- в точках пересечения отрезка и решетки.
\begin{figure}[!htb]
\centering
\includegraphics[width=8.5cm]{section2/diagramPlusIrregular.pdf}
\end{figure}
\end{frame}
...
\includegraphics[scale=0.1]{section2/atoms/atom_4.pdf}
    \caption{Пример ``однократной'' перестройки 4.}
    \label{fig:sect2_atom_4}
    \endminipage\hfill
\minipage{0.5\textwidth}
\centering
\includegraphics[scale=0.225]{section2/atoms/atom_12.pdf}
    \caption{Пример ``двукратной'' перестройки 12.}
    \label{fig:sect2_atom_12}
\endminipage\hfill
\end{figure}
\centering 
Рассмотрены и описаны все 13 критических поверхностей.
\end{frame}
...
\begin{mytheorem}
Пусть $n_1^2<n_2^2$. В области $\left\{\xi < L_1\right\}$ поверхности $\Xi = \xi =  \const$ являются сферами с $2m$ ручками, если $\rho^2(\xi) \in C_m$.
% и сферами с $2m-1$ ручками, если $\rho^2(\xi) \in C_m'$.
\end{mytheorem}

\begin{mytheorem}
Пусть $n_1^2>n_2^2$. В области $\left\{\xi < L_1\right\}$ поверхности $\Xi = \xi =  \const$ являются сферами с $2m+1$ ручками, если $\rho^2(\xi) \in C_m$.
% и сферами с $2m$ ручками, если $\rho^2(\xi) \in C_m'$.
\end{mytheorem}

\vspace{15pt}
Явная формула для $m = m(n_1, n_2, r_1, r_2)$ приведена в работе.

Также рассмотрены особые случаи.
