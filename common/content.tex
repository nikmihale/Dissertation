...
Во \underline{\textbf{введении}} формулируется цель работы, кратко излагаются результаты и содержание. 
...
\begin{align}
& \psi(0,\phi) = \psi(0,-\phi)\notag \\
 &   \qquad\qquad\qquad\qquad     \text{\em (непрерывность сдвига через  $J$)} \label{eq:intro_disp} \tag{1.4}, \\[10pt]
 &   \frac{\partial}{\partial \rho} \psi( \rho, \phi)|_{\rho \to 0} = - \frac{\partial}{\partial \rho} \psi(\rho, - \phi)|_{\rho \to 0} \notag \\
 &\qquad\qquad\qquad\qquad   \text{\em (непрерывность производной через $J$)}\label{eq:intro_grad} \tag{1.5}, 
\end{align}
...
{\small
\begin{align}
& \varkappa^2_{k,m}(\delta) = \frac{\alpha_{\nu, m}^2}{r_0^2} + \delta^2 \frac{\alpha_{\nu, m}^3}{r_0^4} \frac{1}{\frac{\partial W_{\nu,\nu}(u)}{\partial u}}\times \nonumber \\
& \left[
\begin{array}{ll}
\frac{1}{8 \nu} \left( \left. 
\frac{\nu-2}{\nu-1}
...
\left(
W_{\nu+2, \nu}(u) + W_{\nu, \nu+2}(u)
\right)\right)\right|_{u=\alpha_{\nu, m}}, \\
\hspace{28em}   -\nu \in \mathbb{N} \setminus \{1, 2\}; \\
\frac{-1}{12} \left. 
	\left(
	W_{4, 2}(u) + W_{2, 4}(u)
	\right)
\right|_{u=\alpha_{2, m}}, \hspace{14em} -\nu=2; \\
\frac{-3 }{16}
\left. 
	\left(
	W_{3, 1}(u) + W_{1, 3}(u)
	\right)
\right|_{u=\alpha_{1, m}}, \hspace{14em} -\nu=1; \\
\frac{-1}{4} \left. 
 \left( W_{2, 0}(u) + W_{0, 2}(u) \right)
\right|_{u=\alpha_{0, m}}, \hspace{14.75em} \nu=0;   \\
\frac{-1}{16} \left. \left( W_{3, 1}(u) + W_{1, 3}(u) \right)
\right|_{u=\alpha_{1, m}},    \hspace{14.75em} \nu=1;\\
\frac{1}{2} \left. \left(
\frac{1}{4(\nu-1)} \left( W_{\nu-2, \nu}(u) + W_{\nu, \nu-2}(u) \right) - \frac{1}{4(\nu+1)} \left( W_{\nu+2, \nu}(u) + W_{\nu, \nu+2}(u) \right)
\right) \right|_{u=\alpha_{\nu, m}}, \\
\hspace{28em}     \left[
\begin{aligned}
...
Большой раздел второй главы посвящен рассмотрению квантовой свободной частицы в областях двух типов: симметричной $A_\delta$ и несимметричной $B_\delta$ (см. рис. \ref{fig:intro_AB_examples_l} и \ref{fig:intro_AB_examples_r}), где симметрия подразумевается относительно горизонтальной оси $Ox$. 
\begin{figure}[ht]
    \begin{minipage}[b][][b]{0.49\linewidth}\centering
        \includegraphics[width=0.8\linewidth]{right1.pdf}
        \renewcommand{\thefigure}{2.1}
        \caption{Множество $A_\delta$ \\ для  $\phi_0<\pi/2$.}
	\label{fig:intro_AB_examples_l}
%	\tag{2.1}
    \end{minipage}
    \hfill
    \begin{minipage}[b][][b]{0.49\linewidth}\centering
        ...
        \caption{Множество $B_\delta$ \\ для $\phi_0<\phi_1<\pi/2$.}
	\label{fig:intro_AB_examples_r}
%	\tag{2.2}
    \end{minipage}
\end{figure}
\renewcommand{\thefigure}{\arabic{figure}}
Обе области при $\delta \to 0$ имеют своим пределом круговой сектор, результат для которого получен в работе \autocite{wref13}.

Для симметричной области $A_\delta$ вычислена следующая асимптотика собственных значений:
...
Для несимметричной области $B_\delta$  аналогичная асимптотика подчиняется следующей формуле:
...
\centering
   \includegraphics[width=0.9\textwidth]{images/ch4/section1/multiple ellipses.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.4\textwidth]{images/ch4/section2/domain_problemA.pdf}
...
$D_1^6$  	& 	$\alpha_{in} \in (b^2, a^2), \ \alpha_{out} \in (\lambda_1, b^2)$			& сфера с 5 ручками; \\             
\midrule  
$D_2$  	& 	$(\alpha_{out} \in (0, \lambda_1), \ \alpha_{in} < \lambda_1)$ или & \\
		&  $(\alpha_{in} \in (0, \lambda_1), \ \alpha_{out} < \lambda_1)$				& 2 дизъюнктных тора; \\ 
\midrule 
$D_3^1$  	& 	$\alpha_{in} \in (\lambda_1, b^2), \ \alpha_{out} > a^2$				& 2 дизъюнктных тора; \\ \hline 
$D_3^2$  	& 	$\alpha_{in} \in (b^2, a^2), \ \alpha_{out} > a^2 $					& 1 тор; \\             
\midrule 
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.2\textwidth]{images/ch4/section3_circular/domain.pdf}
...
\centering
\includegraphics[width=5.5cm]{images/ch4/section3_circular/n1gtn2.png}
...
\minipage{0.5\textwidth}
\centering
\includegraphics[width=5.5cm]{images/ch4/section3_circular/n1ltn2.png}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.5\textwidth]{images/ch4/section3_circular/line_domains_simple.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\textwidth]{images/ch4/section3_circular/B1_lattice_straight.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.7\textwidth]{images/ch4/section3_circular/B1Prime_lattice.pdf}
...
\renewcommand{\thetheorem}{11}
\begin{theorem}
Пусть $n_1^2<n_2^2$. В области $\left\{\xi < L_1\right\}$ поверхности $S_\Xi$ являются сферами с $2m$ ручками, если $P(\Xi) \in C_m$ и сферами с $2m-1$ ручками, если $P(\Xi) \in C_m'$ и $m \leq \left\lfloor \frac{r_2^2}{r_2^2-r_1^2} \right\rfloor$. 
\label{th:pt10:th1}
\end{theorem}

\renewcommand{\thetheorem}{12}
\begin{theorem}
Пусть $n_1^2 > n_2^2$. В области $\left\{\xi < L_1\right\}$ поверхности $S_\Xi$ являются сферами с $2m+1$ ручками, если $P(\Xi) \in C_m$ и сферами с $2m$ ручками, если $P(\Xi) \in C_m'$ и $m \leq \left\lfloor \frac{r_2^2}{r_2^2-r_1^2} \right\rfloor$. 
\label{th:pt10:th2}
\end{theorem}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.5\textwidth]{images/ch4/section3_circular/sect3_xiL1_subdivision.pdf}
   \renewcommand{\thefigure}{5.28}
    \caption{Разбиение области $\left\{\xi > L_1\right\}$.}
    \label{fig:pt10:_xiL1_subdivision}
\end{figure}
Доказано следующее:
\renewcommand{\thetheorem}{13}
\begin{theorem}
